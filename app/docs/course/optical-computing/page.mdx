...
Before the detectors, a readout mapping is applied:
$$
y = f_3(h_t).
$$

In our optical implementation, $f_1$, $f_2$, and $f_3$ are implemented using diffractive layers
...
1. Build the optical model in SVETlANNa:
     - define detector regions at the output plane (one region per class),
     - implement read-in, memory, and readout diffractive mappings.
...
- Sequence construction: each video is split into subsequences of three frames with a frame interval of two.
- The number of detector regions equals the number of classes:
...
The paper [[1]](https://www.mdpi.com/1424-8220/23/12/5749) does not provide all the low-level
...
One important preprocessing detail is the following:
...
- Tiankuang Zhou et al., “Large-Scale Neuromorphic Optoelectronic Computing with a Reconfigurable Diffractive Processing Unit,” *Nature Photonics* 15, 367–373, 2021.
- Yaze Yu and Yang Cao, “Optical Diffractive Convolutional Neural Networks Implemented in an All-Optical Way,” *Sensors* 23(12), 5749, 2023.
